...
import { METRICS_STORE, type MetricsStore } from './metrics-store.interface';
...
export class MetricsService {
  constructor(@Inject(METRICS_STORE) private readonly store: MetricsStore) {}
  ...
  async compare(tenantId: string, dto: QueryMetricsDto) {
    ...
    return {
      ...
      previous: {
        range: { start: prevStart.toISOString(), end: prevEnd.toISOString() },
        total: previousTotal,
      },
